...
static uint
bmap(struct inode *ip, uint bn, char checksum)
{
  uint addr, *a, x;
  struct buf *bp;
  if(bn < NDIRECT){
  ...
  if(bn < NINDIRECT){
    ...
    if((addr = x) == 0) {
      if (ip->type == T_CHECKED) {   
        ...
        uint ch = checksum << 24;
        ip->addrs[NDIRECT] = ip->addrs[NDIRECT] | ch;
      } else {
...
readi(struct inode *ip, char *dst, uint off, uint n)
{
  ...
  uint i = ip->addrs[off/BSIZE] >> 24;
  char ch = (char) i;

  if (ip->type == T_CHECKED) {
    struct buf *tmp;
    for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
      bp = bread(ip->dev, bmap(ip, off/BSIZE, ch));
      m = min(n - tot, BSIZE - off%BSIZE);
      memmove(tmp->data, bp->data + off%BSIZE, m);
      // brelse(bp);
    
      char checksum = 0;
      int i;
      for (i = 0; i < 512; i++) {
        checksum = checksum^(tmp->data[i]);
      }
...
writei(struct inode *ip, char *src, uint off, uint n)
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    bp = bread(ip->dev, bmap(ip, off/BSIZE,checksum));
    m = min(n - tot, BSIZE - off%BSIZE);
