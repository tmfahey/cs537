...
stati(struct inode *ip, struct stat *st)
{
  if(ip->type == T_CHECKED){
    ...
    for (i=0;i<12;i++){
      ...
      uint b = ((ip->addrs[i] & 0xff000000) >>24);
      check= check ^ (char) b;
    }
    ...
    if(ip->addrs[NDIRECT] !=0) {
      ...
      int j;
      for(j=0;j < 512/sizeof(uint);j++){
          if(a[j] ==0) {
             continue;
          }
          uint c = ((a[j] & 0xff000000));
               c= c >>24;
          check= check ^ (char) c;
      }
...
readi(struct inode *ip, char *dst, uint off, uint n)
{
  ...
  if (ip->type == T_CHECKED) {
    ...
    for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
      ...
      //cprintf("%d\n",ip->addrs[off/BSIZE]);
      if ((uint) checksum != ip->addrs[off/BSIZE] >> 24) {
      // cprintf("checksum incorrect direct\n");
       brelse(bp);
       // brelse(tmp);
	return -1;
      }
        ...
        bp1 = bread(ip->dev, ip->addrs[NDIRECT]);
       uint *a = (uint*)bp1->data;
       uint check = a[(off/BSIZE)-NDIRECT]>>24;
        if(checksum != (char)check){
        /* cprintf("c%d mc%d\n",(uint) checksum,a[(off/BSIZE)-NDIRECT]>>24);
	 cprintf("addwrrd%d \n",a[(off/BSIZE)-NDIRECT]);
         cprintf("check wrong in read indirect\n");
         int k;
         for(k=0;k<512;k++){
         cprintf(" %d %d",k,bp->data[k]);
          }
	 cprintf("\n");*/
          cprintf("checksum incorrect indirect\n");
          brelse(bp1);
          brelse(bp);
	return -1;
      }
...
writei(struct inode *ip, char *src, uint off, uint n)
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    ...
    ip->addrs[off/BSIZE] = ip->addrs[off/BSIZE] | ch;
    //cprintf("ch|addr%d\n ",ip->addrs[off/BSIZE]);
    }
    else {
    bp1 = bread(ip->dev, ip->addrs[NDIRECT]);
    a = (uint*)bp1->data;
    a[(off/BSIZE)-NDIRECT]= a[(off/BSIZE)-NDIRECT] & 0x00FFFFFF;
    a[(off/BSIZE)-NDIRECT]= a[(off/BSIZE)-NDIRECT]| ch;
    /*if((off/BSIZE)-NDIRECT==0){
    cprintf("write check indirect 12\n");
    cprintf(" c %d mc %d in write\n",(int)checksum,a[(off/BSIZE)-NDIRECT]);
    cprintf("addwrwr%d \n",a[(off/BSIZE)-NDIRECT]);
    int k;
    for(k=0;k<512;k++){
    cprintf(" %d %d",k,bp->data[k]);
