...
count = 4096;
      ...
      int fill = 4096 - left;
    // case 1: the whole block is filled
    if ((left) < count)
	return -1;*/
      //case 2: the whole block is not filled
   ...
   // printf("%d inode loca create1\n",map->inode[pinum % 16]);
    ...
    for( i=0;i<= nublock;i++) {
	  ...
	  //printf("name %s\n",dirent->name);
        ...
        //printf("%d create1 nublock\n",nublock);
	//printf("%d %d %d create1\n",bl_off,block_num,off);
...
int s_unlink(int pinum, char *name){
    ...
    for( i=0;i<= nublock;i++) {
       ...
       for(j=0;j<4096/sizeof(MFS_DirEnt_t);j++) {
	    ...
	    inode_map *map1 = malloc(sizeof(inode_map));
	    read(fd, map1,sizeof(inode_map));
	    offset = map1->inode[itmp % 16];
            if(itmp % 16==2){
            fprintf(stderr,"inode ptr in unlink %d\n",map1->inode[itmp % 16]);
             }
...
void dump() {
    ...
    int nodec = 0;
    for (i = 0; i < 1; i ++) {
	printf("imap %d, at offset %d  | ",i, check->ptr[i]);
	lseek(fd, check->ptr[i], SEEK_SET);
   	inode_map *imap = malloc(sizeof(inode_map));
   	read(fd, imap, sizeof(inode_map));
	int j;
	for(j = 0; j < 2; j++) {
	     printf("inode %d at offset %d",nodec, imap->inode[j]);
	     lseek(fd, imap->inode[j], SEEK_SET);
   	     MFS_inode_t *inode = malloc(sizeof(MFS_inode_t));
   	     read(fd, inode, sizeof(MFS_inode_t));
	     printf(" type %d size %d offset of ptr[0] %d",inode->type, inode->size, inode->ptr[0]);
             int k;
